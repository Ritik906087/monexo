...
export default function LinkNewUPIPage() {
  ...
  const handleLinkKyc = async () => {
    if (!name || !upiNo || !linkedMobile) {
      toast({ variant: "destructive", title: "Missing Fields", description: "Please fill all details correctly." });
      return;
    }
  ...
  return (
    <div className="flex flex-col h-full bg-white animate-slide-up relative overflow-hidden">
      <div className="bg-white h-[56px] flex items-center px-4 shrink-0 border-b border-slate-50 relative">
        <button onClick={() => router.back()} className="w-10 h-10 flex items-center justify-center -ml-2 active:scale-90 transition-all">
      ...
      <div className="flex-1 p-5 space-y-0">
        <div onClick={() => setIsSheetOpen(true)} className="flex items-center justify-between py-4 border-b border-slate-100 active:bg-slate-50 transition-colors cursor-pointer">
          <div className="flex items-center gap-4 w-full">
            <span className="text-[15px] text-slate-800 w-24 shrink-0 font-medium">Partner</span>
            <div className="flex items-center justify-between flex-1">
              <span className={cn("text-[16px] font-black tracking-tight", selectedPartner ? "text-slate-900" : "text-slate-300")}>
        ...
        {selectedPartner && (
          <div className="animate-slide-up space-y-2">
            <div className="flex items-center py-4 border-b border-slate-100">
              <span className="text-[15px] text-slate-800 w-24 shrink-0 font-medium">Name</span>
              <Input 
            ...
            <div className="flex items-center py-4 border-b border-slate-100">
              <span className="text-[15px] text-slate-800 w-24 shrink-0 font-medium">UPI No</span>
              <Input 
            ...
            <div className="flex items-center py-4 border-b border-slate-100">
              <span className="text-[15px] text-slate-800 w-24 shrink-0 font-medium leading-tight">Linked Mobile</span>
              <Input 
